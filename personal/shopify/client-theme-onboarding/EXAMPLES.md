...
Four of the six categories, shown as filled rows. The complete set of headings, in order, is the block in `references/components-format.md`.
...
| name | file path(s) | what it does | reuse keywords |
|---|---|---|---|
| scroll-reveal | `theme/_qs/js/src/utils/reveal.ts` · `.reveal` classes in `theme/_qs/scss/base/_animations.scss` | scroll · IntersectionObserver adds `.is-visible`, CSS fades/translates in — used across landing sections | reveal, scroll, fade in, entrance, animate on scroll |
| card-hover-lift | `theme/_qs/scss/components/_card.scss` (`.c-card`) | hover · CSS transition — card lifts + shadow, shared by every card grid | hover, lift, card, shadow |
```
